...
input_file = r"C:\Users\Administrator\Documents\GitHub\Text_cleaning\data\output_data\root1\http___www.rajbhavan.kerala.gov.in_index.php_medio_pressrelease2_cleaned.json"
...
try:
    with open(input_file, "r", encoding="utf-8") as infile:
        try:
            ...
            data = None #set data to none to avoid further errors

    if data: #check if data loaded correctly
        if isinstance(data.get("content"), list): #use .get to handle missing key
            augmented_contents = []
            for content_item in data["content"]:
                ...
                if preprocessed_item:
                    ...
                    augmented_contents.append(augmented_item if augmented_item else content_item)
                else:
        ...
        if data and "augmented_content" in data and data["augmented_content"] is not None: #check if data and content is not none
            with open(output_file, "w", encoding="utf-8") as outfile:
                json.dump(data, outfile, ensure_ascii=False, indent=4)
                print("Data augmentation complete. Augmented file saved to:", output_file)
        else:
            print("Warning: Data or augmented_content is empty or None. No file written.")
    else:
